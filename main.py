...
class Juego(FloatLayout):
    ...
    def _resize(self, *args):
        ...
        for panel, bg in [(self.stats_panel, self.stats_bg), (self.debug_panel, self.debug_bg)]:
            panel.size = (ancho_p, alto_p); panel.pos = (pos_x, pos_y)
            bg.size = (ancho_p, alto_p); bg.pos = (pos_x, pos_y)
    ...
    def _crear_ui_paneles(self):
        ...
        with self.stats_panel.canvas.before:
            Color(0, 0, 0, 1); self.stats_bg = RoundedRectangle(radius=[25])
        self.stats_label = Label(text="", color=(1,1,1,1), halign="left", valign="top", size_hint=(.9, .6), pos_hint={"center_x": .5, "center_y": .6})
        self.stats_label.bind(size=self.stats_label.setter("text_size"))
        self.btn_cerrar_stats = Button(text="Cerrar", size_hint=(None, None), size=(180, 60), pos_hint={"center_x": .5, "y": .1})
        self.btn_cerrar_stats.bind(on_release=self.ocultar_paneles)
        self.stats_panel.add_widget(self.stats_label); self.stats_panel.add_widget(self.btn_cerrar_stats)
        self.add_widget(self.stats_panel)
        
        # Debug - RESTAURADAS LAS 10 OPCIONES
        ...
        self.btn_cerrar_debug = Button(text="Cerrar Debug", size_hint=(None, None), size=(180, 55), pos_hint={"center_x": .5, "y": .05})
    ...
    def on_touch_down(self, touch):
        if self.stats_panel.opacity > 0:
            if self.btn_cerrar_stats.collide_point(*touch.pos): self.ocultar_paneles(); return True
            return True
        if self.debug_panel.opacity > 0:
            if self.btn_cerrar_debug.collide_point(*touch.pos): self.ocultar_paneles(); return True
    ...
    def ocultar_paneles(self, *_): self.stats_panel.opacity = self.debug_panel.opacity = 0; self.stats_panel.disabled = self.debug_panel.disabled = True; self.paused = False
    ...
    def forzar_evento(self, t):
        self.total_eventos += 1; self.evento_timer, self.lbl_evento.text = EVENTO_DURACION, f"Evento: {t}"
    ...
    def update(self, dt):
        self.bg_hue = (self.bg_hue + dt * 0.02) % 1; self.bg_color.hsv = (self.bg_hue, 0.1, 1)
